 ...
 * Optional tags
 */
class OptionalTagsTest extends BaseTest
{
	/**
		...
		foreach (array_keys($block_tags) as $block) {
			foreach (array_keys($optional_closing_tags) as $e) {
				foreach (array_keys($optional_closing_tags[$e]) as $child) {
					...
					$data["$block should close nested $e and $child"] = [
						"<$block><$e><$child></$block>",
		...
		foreach (array_keys($block_tags) as $block) {
			foreach (array_keys($optional_closing_tags) as $e) {
				foreach (array_keys($optional_closing_tags[$e]) as $child) {
					...
					$data["root should close nested $e and $child"] = [
						"<$e><$child>",
						"<$e><$child></$child></$e>",
