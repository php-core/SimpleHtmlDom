...
namespace PHPCore\SimpleHtmlDom;
...
class Debug {
	...
	private static mixed $debugHandler = null;
	private static array $callerLock = [];
	 ...
	 */
	public static function setDebugHandler($function = null): void
	{
	 ...
	 */
	private static function log_trace($message, $backtrace): void
	{
		$idx = 0;
		$debugMessage = '';
		...
		foreach($backtrace as $caller)
		{
			...
			if (!isset($caller['class']) || strpos($caller['class'], 'PHPCore\SimpleHtmlDom\\') !== 0) {
				break;
			}
	 ...
	 * @param string $message The message to add to error_log
	 */
	public static function log_once(string $message): void
	{
		if (!self::isEnabled()) return;
